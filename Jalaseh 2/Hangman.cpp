...
bool writeWord(string word, vector<char> correctLetters,
               vector<char> wrongLetters);
bool isLetterInVector(vector<char> letters, char letter);
bool drawBoard(vector<char> &wrongLetters, vector<char> &correctLetters,
               string word);
string getLower(string s);
void getInput(vector<char> &wrongLetters, vector<char> &correctLetters,
              string word, bool &lost);
...
bool writeWord(string word, vector<char> correctLetters,
  ...
  for (int i = 0; i < word.length(); i++) {
    if (find(correctLetters.begin(), correctLetters.end(), tolower(word[i])) !=
        correctLetters.end()) {
      cout << word[i];
      found = true;
    }
  ...
  if (wrongLetters.size() > 0) {
    cout << endl << "Wrong guessed letters:";
...
bool drawBoard(vector<char> &wrongLetters, vector<char> &correctLetters,
  ...
  if ((wrongLetters.size() >= 6 and !complete) || lost) {
    cout << "You lost." << endl << "The secret word was " << word << endl;
    complete = true;
  } else if (complete) {
    cout << "You won." << endl << "The secret word was " << word << endl;
  }
  return complete;
}
string getLower(string s) {
...
void getInput(vector<char> &wrongLetters, vector<char> &correctLetters,
  ...
  while (!correct) {
    ...
    if (tolower(choice) == 'y') {
      ...
      if (!isLetterInVector(correctLetters, inputChar) &&
          !isLetterInVector(wrongLetters, inputChar)) {
