...
void initScreen(int n) {
  ...
  while (true) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = i; j < n; j++) {
        if ((shapes[i].first->getCenter() / shapes[j].first->getCenter()) <=
            shapes[i].first->getRadius() + shapes[j].first->getRadius()) {
          std::pair<float, float> speedI = shapes[i].first->getSpeed();
          std::pair<float, float> speedJ = shapes[j].first->getSpeed();
          shapes[i].first->setSpeed(
              std::pair<float, float>(-speedI.first, -speedI.second));
          shapes[j].first->setSpeed(
              std::pair<float, float>(-speedJ.first, -speedJ.second));
        }
      }
    }
  }
