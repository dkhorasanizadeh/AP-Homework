...
Signup::Signup(QWidget *parent) : QMainWindow(parent) {
  ...
  enterSignup = new QShortcut(QKeySequence(Qt::Key::Key_Enter), this,
                              SLOT(signupButtonPressed()));
...
void Signup::openSignup(void) { this->show(); }
  ...
  if (firstNameLineEdit->text().length() == 0 ||
    ...
    if (passwordLineEdit->text() != passwordRepeatLineEdit->text()) {
      ...
      userManager->addNewUser(
          firstNameLineEdit->text(), lastNameLineEdit->text(),
          usernameLineEdit->text(), passwordLineEdit->text(),
          emailLineEdit->text(), ageSpinBox->value(),
